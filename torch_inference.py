...
with pm.torch_profile_section():
    with pm.torch_profile_function("Torch Inference"):
        ...
        with pm.torch_profile_function("Embedding Layer"):
            embedding_layer = torch.nn.Embedding(vocab_size, dim, device=DEVICE)
            embedding_layer.weight.data.copy_(model["tok_embeddings.weight"])
            token_embeddings_unnormalized = embedding_layer(tokens).to(TORCH_FLOAT)


        def rms_norm(tensor, norm_weights):
            return (tensor * torch.rsqrt(tensor.pow(2).mean(-1, keepdim=True) + norm_eps)) * norm_weights

        with pm.torch_profile_function("Q Layer Prep"):
            q_layer0 = model["layers.0.attention.wq.weight"]
            head_dim = q_layer0.shape[0] // n_heads
            q_layer0 = q_layer0.view(n_heads, head_dim, dim)

        with pm.torch_profile_function("Freqs Prep"):
            zero_to_one_split_into_64_parts = (torch.tensor(range(64)) / 64).to(DEVICE)
            freqs = 1.0 / (rope_theta ** zero_to_one_split_into_64_parts)
            freqs_for_each_token = torch.outer(torch.arange(17).to(DEVICE), freqs)
            freqs_cis = torch.polar(torch.ones_like(freqs_for_each_token).to(DEVICE), freqs_for_each_token)

        with pm.torch_profile_function("K and V Layers Prep"):
        ...
        for layer in range(n_layers):
            with pm.torch_profile_function(f"Layer {layer}"):
                qkv_attention_store = []
                ...
                for head in range(n_heads):
                    with pm.torch_profile_function(f"Layer {layer} Attention Head {head}"):
        ...
        with pm.torch_profile_function("Decode Token"):
            final_embedding = rms_norm(final_embedding, model["norm.weight"])
            logits = torch.matmul(final_embedding[-1], model["output.weight"].T)
            next_token = torch.argmax(logits, dim=-1)
            tokenizer.decode([next_token.item()])
            # 42
            print("tokenizer.decode([next_token.item()])", tokenizer.decode([next_token.item()]))
